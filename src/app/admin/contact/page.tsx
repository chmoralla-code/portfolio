    ...
    <div>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <div className="card space-y-4">
          ...
          <div>
            <label className="mb-1.5 block text-sm font-medium text-zinc-400">Phone</label>
            <input type="text" value={data.contact.phone || ""} onChange={e => setData({ ...data, contact: { ...data.contact, phone: e.target.value } })} />
          </div>
          <div>
            <label className="mb-1.5 block text-sm font-medium text-zinc-400">Location</label>
            <input type="text" value={data.contact.location || ""} onChange={e => setData({ ...data, contact: { ...data.contact, location: e.target.value } })} />
          </div>
          <div>
            <label className="mb-1.5 block text-sm font-medium text-zinc-400">Availability</label>
        ...
        <div>
          ...
          <div className="space-y-3">
            ...
            {data.socialLinks.length === 0 && <p className="text-center text-zinc-500">No social links yet.</p>}
          </div>
        </div>
      </div>
    </div>
